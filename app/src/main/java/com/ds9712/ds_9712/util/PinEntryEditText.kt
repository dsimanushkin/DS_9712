...
class PinEntryEditText : AppCompatEditText {
    private var mSpace = 12f //24 dp by default, space between the lines
    private var mCharSize = 0f
    private var mNumChars = 6f
    private var mLineSpacing = 18f //8dp by default, height of the text from our lines
    private var mMaxLength = 6
    private var mClickListener: OnClickListener? = null
    private var mLineStroke = 4f //1dp by default / Thickness of bottom stroke in unselected state
    private var mLineStrokeSelected = 4f //2dp by default / Thickness of bottom stroke in selected state
    private var mLinesPaint: Paint? = null
    private var darkBoxPaint: Paint? = null
    var mStates = arrayOf(
    ...
    private fun init(context: Context, attrs: AttributeSet) {
        ...
        darkBoxPaint = Paint(paint)
        darkBoxPaint!!.strokeWidth = 50f
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        while (i < mNumChars) {
            ...
            if (getText()!!.length > i) {
                val charSize = (availableWidth - mSpace * (mNumChars - 1)) / mNumChars / 2
                val middle = startX + charSize
                if (i == 0) {
                    canvas.drawText(text, i, i + 1, charSize, bottom - mLineSpacing, textPaint)
                } else {
                    canvas.drawText(text, i, i + 1, middle, bottom - mLineSpacing, textPaint)
                }
            }
            startX += if (mSpace < 0) {
                (mCharSize * 2).toInt()
    ...
    private fun updateColorForLines(next: Boolean) {
        if (isFocused) {
            ...
            darkBoxPaint!!.strokeWidth = 550f
            darkBoxPaint!!.color = Color.parseColor("#26BCBCBC")
            if (next) {
                mLinesPaint!!.color = Color.parseColor("#FFFFFF")
            }
        } else {
            mLinesPaint!!.strokeWidth = mLineStroke
            mLinesPaint!!.color = Color.parseColor("#FFFFFF")
